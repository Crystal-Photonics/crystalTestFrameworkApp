...
IsotopeSourceSelector::IsotopeSourceSelector(UI_container *parent)
    : UI_widget{parent}, combobox{new QComboBox(parent)} {
...
double IsotopeSourceSelector::get_selected_activity_Bq() {
  ...
  save_most_recent();
  return get_source_by_serial_number(combobox->currentText())
      .get_activtiy_becquerel(QDate::currentDate());
}
...
std::string IsotopeSourceSelector::get_selected_name() {
  auto isot = get_source_by_serial_number(
      QString::fromStdString(get_selected_serial_number()));
...
void IsotopeSourceSelector::fill_combobox_with_isotopes(QString isotope_name) {
  ...
  for (auto item : isotope_sources) {
    if ((isotope_name == "") || (isotope_name == "*") ||
        (item.isotope.toLower() == isotope_name.toLower())) {
...
IsotopeSource
IsotopeSourceSelector::get_source_by_serial_number(QString serial_number) {
  for (auto item : isotope_sources) {
    if (item.serial_number == serial_number) {
      return item;
    }
  }
  QString msg = QString{"cant find isotope source serial_number \"%1\""}.arg(
      serial_number);
  throw sol::error(msg.toStdString());
}

void IsotopeSourceSelector::load_isotope_database() {
  QString fn = QSettings{}
                   .value(Globals::isotope_source_data_base_path_key, "")
                   .toString();
  ...
  for (auto item : jarray) {
    ...
    isotope_source.start_activity_becquerel =
        obj["start_activity_kBq"].toDouble() * 1000;
    if ((isotope_source.isotope.toLower() == "co57") ||
        (isotope_source.isotope.toLower() == "co-57")) {
      // 271 Tage 17 Stunden
      isotope_source.half_time_days = 271 + 17 / 24;

    } else if ((isotope_source.isotope.toLower() == "na22") ||
               (isotope_source.isotope.toLower() == "na-22")) {
      // 2.6 Years
      isotope_source.half_time_days = 2.6 * 365;

    } else if ((isotope_source.isotope.toLower() == "j129") ||
               (isotope_source.isotope.toLower() == "j-129") ||
               (isotope_source.isotope.toLower() == "i129") ||
               (isotope_source.isotope.toLower() == "i-129")) {
      // 1,57 * 10^7  years;
      isotope_source.half_time_days = 1.57e7 * 365;
    } else {
      QString msg = QString{"dont knwo halftime of isotope \"%1\" in isotope "
                            "source database."}
...
void IsotopeSourceSelector::load_most_recent() {
  assert(
      ...
      QThread::currentThread()); // event_queue_run_ must not be started by the
                                 // GUI-thread because it would freeze the GUI
  QString most_recent_serial_number =
      QSettings{}
          .value(QString(Globals::isotope_source_most_recent_key) + "_" +
                     isotope_filter_m.toLower(),
                 "")
          .toString();
...
void IsotopeSourceSelector::save_most_recent() {
  assert(
      ...
      QThread::currentThread()); // event_queue_run_ must not be started by the
                                 // GUI-thread because it would freeze the GUI
  QSettings{}.setValue(QString(Globals::isotope_source_most_recent_key) + "_" +
                           isotope_filter_m.toLower(),
...
void IsotopeSourceSelector::connect_isotope_selecteted() {
  ...
  callback_isotope_selected = QObject::connect(
      combobox, &QComboBox::currentTextChanged, [this](const QString &text) {
        save_most_recent();
        (void)text;
      });
}
...
void IsotopeSourceSelector::set_single_shot_return_pressed_callback(
...
double IsotopeSource::get_activtiy_becquerel(QDate date_for_activity) {
  ...
  double current_activity =
      (start_activity_becquerel * std::exp(-days * ln2 / half_time_days));

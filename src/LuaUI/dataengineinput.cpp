...
DataEngineInput::DataEngineInput(
    UI_container *parent_, ScriptEngine *script_engine,
    Data_engine *data_engine_, std::string field_id_,
    std::string extra_explanation_, std::string empty_value_placeholder_,
    std::string desired_prefix_, std::string actual_prefix_)
    : UI_widget{parent_}, label_extra_explanation{new QLabel(parent_)},
      label_de_description{new QLabel(parent_)},
      label_de_desired_value{new QLabel(parent_)},
      label_de_actual_value{new QLabel(parent_)}, label_ok{new QLabel(parent_)},
      timer{new QTimer(parent_)},
      data_engine{data_engine_}, field_id{QString::fromStdString(field_id_)},
  ...
  assert(
      MainWindow::gui_thread ==
      QThread::currentThread()); // event_queue_run_ must not be started by the
  ...
  QString s = this->desired_prefix + " " +
              data_engine->get_desired_value_as_string(this->field_id);
  label_de_desired_value->setText(s);
  label_de_desired_value->setWordWrap(true);
  label_de_actual_value->setText(this->actual_prefix + " " +
                                 this->empty_value_placeholder);
  ...
  default:
    throw std::runtime_error(
        QObject::tr(
            "DataEngineInput for field-id \"%1\" does not yet support Field "
            "types other than Bool, Number and Text but is: \"%2\"")
            .arg(field_id)
            .arg(entry_type.to_string())
            .toStdString());
    break;
  }
  ...
  label_next = new QLabel(
      "(or " +
          QSettings{}.value(Globals::confirm_key_sequence_key, "").toString() +
          ")",
      parent_);
  ...
  label_no = new QLabel(
      "(or " +
          QSettings{}.value(Globals::cancel_key_sequence_key, "").toString() +
          ")",
      parent_);
  ...
  button_exceptional_approval =
      new QPushButton(QObject::tr("Exceptional approval"), parent_);
  ...
  callback_bool_no =
      QObject::connect(button_no, &QPushButton::clicked, [this]() {
        this->script_engine->post_hotkey_event(Event_id::Hotkey_cancel_pressed);
      });
  callback_bool_yes =
      QObject::connect(button_yes, &QPushButton::clicked, [this]() {
        this->script_engine->post_hotkey_event(
            Event_id::Hotkey_confirm_pressed);
      });
  callback_next =
  ...
  callback_exceptional_approval = QObject::connect(
      button_exceptional_approval, &QPushButton::clicked, [this]() {
        ExceptionalApprovalDB ea_db{
            QSettings{}
                .value(Globals::path_to_excpetional_approval_db_key, "")
                .toString()};
        if (data_engine->do_exceptional_approval(ea_db, field_id,
                                                 MainWindow::mw)) {
...
void DataEngineInput::save_to_data_engine() {
  ...
  if (is_editable) {
    ...
    case EntryType::Number: {
      ...
      if (!ok) {
        val = QInputDialog::getDouble(
            ...
            "Der Wert \"" + t + "\" im Feld \"" + label_de_description->text() +
    ...
    throw std::runtime_error(
        QObject::tr(
            "DataEngineInput for field-id \"%1\" shall write to dataengine but "
            "since it is not in edit mode, there is no data to "
            "write. The functions save_to_data_engine, load_actual_value and "
            "await_event set the editable flag to false.")
            .arg(field_id)
            .toStdString());
  }
...
void DataEngineInput::set_explanation_text(
    const std::string &extra_explanation) {
  assert(MainWindow::gui_thread == QThread::currentThread());
  this->extra_explanation = QString::fromStdString(extra_explanation);
  label_extra_explanation->setText(this->extra_explanation);
}

bool DataEngineInput::get_is_editable() { return is_editable; }
...
void DataEngineInput::start_timer() {
  callback_timer = QObject::connect(timer, &QTimer::timeout, [this]() {
    if (blink_state == Globals::ui_blink_ratio) {
      ...
      label_de_actual_value->setText(actual_prefix + " " +
      ...
      label_extra_explanation->setText(extra_explanation);
      label_extra_explanation->setFixedHeight(
          label_extra_explanation->height());
    }
    blink_state++;
  });
}
...
void DataEngineInput::set_total_visibilty() {
  ...
  label_ok->setVisible(is_visible &&
                       ((is_editable == false) || (is_enabled == false)));
...
void DataEngineInput::set_ui_visibility() {
  Utility::promised_thread_call(MainWindow::mw, [this] {
    assert(MainWindow::gui_thread ==
           QThread::currentThread()); // event_queue_run_ must not be started by
                                      // the GUI-thread because it would freeze
                                      // the GUI
    if (init_ok == false) {
      return;
    }
    ...
    if (is_waiting) {
      ...
      label_extra_explanation->setVisible(is_visible && is_enabled &&
                                          is_waiting);
      return;
    }
    set_labels_enabled();

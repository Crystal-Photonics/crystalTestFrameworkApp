...
DataEngineInput::DataEngineInput(UI_container *parent_, ScriptEngine *script_engine, Data_engine *data_engine_, std::string field_id_,
	...
	switch (entry_type.t) {
		...
		default:
			throw std::runtime_error(QObject::tr("DataEngineInput for field-id \"%1\" does not yet support Field "
...
void DataEngineInput::save_to_data_engine() {
	...
	if (is_editable) {
		...
		throw std::runtime_error(QObject::tr("DataEngineInput for field-id \"%1\" shall write to dataengine but "
...
void DataEngineInput::set_ui_visibility() {
	Utility::promised_thread_call(MainWindow::mw, [this] {
		assert(MainWindow::gui_thread == QThread::currentThread()); // event_queue_run_ must not be started by

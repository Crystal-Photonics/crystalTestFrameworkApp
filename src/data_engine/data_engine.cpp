...
Data_engine::Data_engine(std::istream &source,
                         const QMap<QString, QList<QVariant>> &tags)
    : sections{} {
  set_dependancy_tags(tags);
  set_source(source);
}

Data_engine::Data_engine(std::istream &source) : sections{} {
  // this constructor is used to work in dummy mode
  // dummy mode means that no dependency tags were defined in order to get just
  // the structure of the data engine source file. for example for report
  // templates or report queries.
...
void Data_engine::enable_logging(Console &console,
                                 const std::vector<MatchedDevice> &devices) {
...
void Data_engine::set_dependancy_tags(
    const QMap<QString, QList<QVariant>> &tags) {
...
void Data_engine::set_source(std::istream &source) {
  QByteArray data;
  constexpr auto eof =
      std::remove_reference<decltype(source)>::type::traits_type::eof();
  while (source) {
  ...
  if (!document.isObject()) {
    throw DataEngineError(
        DataEngineErrorNumber::invalid_json_file,
        "Dataengine: Invalid JSON file.\n\nYou can use one of the online JSON "
        "validators you will find at google.");
  }
  ...
  for (const auto &section :
       sections.sections) { // lets test whether field names with the same name
                            // across variants have also the same type
...
void Data_engine::start_recording_actual_value_statistic(
    const std::string &root_file_path, const std::string &file_prefix) {
  statistics_file.start_recording(QString::fromStdString(root_file_path),
                                  QString::fromStdString(file_prefix));
}
...
void Data_engine::set_start_time_seconds_since_epoch(
    double start_seconds_since_epoch) {
  this->load_time_seconds_since_epoch =
      static_cast<qint64>(start_seconds_since_epoch);
}
...
void DataEngineSections::set_instance_count(QString instance_count_name,
                                            uint instance_count) {
  bool instance_count_name_found = false;
  for (auto &section : sections) {
    if (section.set_instance_count_if_name_matches(instance_count_name,
                                                   instance_count)) {
      if (is_dummy_data_mode) {
        section.delete_all_but_biggest_variants();
      } else {
  ...
  if (instance_count_name_found == false) {
    throw DataEngineError(
        DataEngineErrorNumber::instance_count_does_not_exist,
        QString("Dataengine: Could not find instance count name:\"%1\"")
            .arg(instance_count_name));
  }
}

void DataEngineSections::use_instance(QString section_name,
  ...
  if (error_num == DataEngineErrorNumber::no_section_id_found) {
    throw DataEngineError(
        ...
        QString("Dataengine: Could not find section with name = \"%1\"")
            .arg(section_name));
  }
...
DataEngineSection *DataEngineSections::get_section(const FormID &id) const {
  ...
  DataEngineSection *result_section =
      get_section_raw(field_id.section_name, &error_num);
  if (result_section == nullptr)
    throw DataEngineError(DataEngineErrorNumber::no_section_id_found,
                          "Dataengine: Section not found (" +
                              field_id.section_name + ")");
  return result_section;
}

DataEngineSection *
DataEngineSections::get_section_no_exception(FormID id) const {
  DataEngineErrorNumber error_num = DataEngineErrorNumber::ok;
  if (!id.contains("/")) {
    id += "/dummy";
  }
...
DecodecFieldID DataEngineSections::decode_field_id(const FormID &id) {
  ...
  if (names.count() != 2) {
    throw DataEngineError(DataEngineErrorNumber::faulty_field_id,
                          QString("Dataengine: field id needs to be in format "
                                  "\"section-name/field-name\" but is %1")
                              .arg(id));
  }
...
void DataEngineSections::set_dependancy_tags(
    const QMap<QString, QList<QVariant>> &tags) {
  dependency_tags = tags;
}

const QMap<QString, QList<QVariant>> &
DataEngineSections::get_dependancy_tags() const {
  return dependency_tags;
}

DataEngineSection *
DataEngineSections::get_section_raw(const QString &section_name,
  ...
  *error_num = DataEngineErrorNumber::ok;
  for (auto &section : sections) {
    if (section.get_section_name().toLower() == section_name.toLower()) {
      assert(result ==
             nullptr); // there should only be one section with this name
      result = &section;
    }
  }
  if (result == nullptr) {
...
QList<const DataEngineDataEntry *> DataEngineSections::get_entries_raw(
    const FormID &id, DataEngineErrorNumber *error_num,
    DecodecFieldID &decoded_field_name, bool using_instance_index,
    bool using_dummy_mode) const {
  *error_num = DataEngineErrorNumber::ok;
  decoded_field_name = decode_field_id(id);
  DataEngineSection *section =
      get_section_raw(decoded_field_name.section_name, error_num);

  QList<const DataEngineDataEntry *> result;

  if (section) {
    if ((section->is_section_instance_defined() == false) &&
        (using_dummy_mode == false)) {
      assert(0); // exception should already have been thrown elsewere
      // throw
      // DataEngineError(DataEngineErrorNumber::instance_count_yet_undefined,
      //                      QString("Dataengine: Instance count of section
      //                      \"%1\" yet undefined. Instance count value is:
      //                      \"%2\".")
    ...
    if (using_dummy_mode) {
      for (const VariantData &variant : section->prototype_instance.variants) {
        ...
        const DataEngineDataEntry *item =
            variant.get_entry_raw(decoded_field_name.field_name, &errornum);
      ...
      for (uint index = 0; index < section->instances.size(); index++) {
        ...
        if (using_instance_index &&
            (index != section->get_actual_instance_index())) {
          continue;
        }
        const VariantData *variant = instance.get_variant();
        assert(variant);
        DataEngineErrorNumber error_num_dummy;
        const DataEngineDataEntry *item = variant->get_entry_raw(
            decoded_field_name.field_name, &error_num_dummy);
        if (item) {
          result.append(item);
        }
      }
...
DataEngineSections::get_entries_const(const FormID &id,
                                      const bool using_dummy_mode) const {
  DataEngineErrorNumber error_num = DataEngineErrorNumber::ok;
  ...
  if (error_num == DataEngineErrorNumber::no_field_id_found) {
    throw DataEngineError(
        DataEngineErrorNumber::no_field_id_found,
        QString("Dataengine: Could not find field with name = \"%1\"")
            .arg(decoded_field_id.field_name));
  } else if (error_num == DataEngineErrorNumber::no_section_id_found) {
    throw DataEngineError(
        DataEngineErrorNumber::no_section_id_found,
        QString("Dataengine: Could not find section with name = \"%1\"")
            .arg(decoded_field_id.section_name));
...
QList<const DataEngineDataEntry *>
DataEngineSections::get_entries_across_variants_const(const FormID &id) const {
  DataEngineErrorNumber error_num = DataEngineErrorNumber::ok;
  DecodecFieldID decoded_field_id;
...
DataEngineSections::get_actual_instance_entry_const(const FormID &id) const {
  DataEngineErrorNumber error_num = DataEngineErrorNumber::ok;
  DecodecFieldID decoded_field_id;
  auto result_list =
      get_entries_raw(id, &error_num, decoded_field_id, true, false);
  if (error_num == DataEngineErrorNumber::no_field_id_found) {
    throw DataEngineError(
        DataEngineErrorNumber::no_field_id_found,
        QString("Dataengine: Could not find field with name = \"%1\"")
...
bool DataEngineSections::exists_uniquely(const FormID &id,
                                         const bool using_dummy_mode) const {
  DataEngineErrorNumber error_num = DataEngineErrorNumber::ok;
  ...
  for (const auto &key : object.keys()) {
    ...
    if (!(section_object.isObject() || section_object.isArray())) {
      throw std::runtime_error(
          QString("Dataengine: invalid json object %1").arg(key).toStdString());
    }
    DataEngineSection section;
    section.from_json(section_object, key);
    if (section_exists(section.get_section_name())) {
      throw DataEngineError(DataEngineErrorNumber::duplicate_section,
                            QString("Dataengine: duplicate section %1")
                                .arg(section.get_section_name()));
    }
...
QStringList DataEngineSection::get_all_ids_over_variants() const {
  ...
  for (const auto &variant : prototype_instance.variants) {
    for (const auto &data_entry : variant.data_entries) {
      QString id =
          section_name.toLower() + "/" + data_entry->field_name.toLower();
...
EntryType DataEngineSections::get_entry_type_dummy_mode_recursion(
    const FormID &id) const {
  ...
  if (data_entries_for_each_variant.count() == 0) {
    throw DataEngineError(DataEngineErrorNumber::no_field_id_found,
                          "Dataengine: field not found: " + id);
  }

  const DataEngineDataEntry *entry = data_entries_for_each_variant[0];
  const ReferenceDataEntry *referece_entry =
      dynamic_cast<const ReferenceDataEntry *>(entry);
  ...
  if (referece_entry) {
    ...
    for (const auto &ref_link : referece_entry->reference_links) {
      if (first) {
        ...
        if (result_type.t !=
            get_entry_type_dummy_mode_recursion(ref_link.link).t) {
          throw DataEngineError(
              DataEngineErrorNumber::
                  inconsistant_types_across_variants_and_reference_targets,
              "Dataengine: Inconsistent type across variants and references. "
              "field: " +
                  id);
        }
      }
    ...
    for (auto &data_entry : data_entries_for_each_variant) {
      if (first) {
        ...
        if (result_type.t != data_entry->get_entry_type().t) {
          throw DataEngineError(
              DataEngineErrorNumber::inconsistant_types_across_variants,
              "Dataengine: Inconsistent types across variants for field: " +
                  id);
        }
      }
    }
...
EntryType
DataEngineSections::get_entry_type_dummy_mode(const FormID &id) const {
...
void DataEngineSection::delete_unmatched_variants(
    const QMap<QString, QList<QVariant>> &tags) {
...
DataEngineInstance::DataEngineInstance(const DataEngineInstance &other)
    : variants{other.variants}, instance_caption{other.instance_caption},
      section_name{other.section_name}, allow_empty_section{
                                            other.allow_empty_section} {}

DataEngineInstance::DataEngineInstance(DataEngineInstance &&other)
    : variants(std::move(other.variants)), instance_caption{std::move(
                                               other.instance_caption)},
...
const VariantData *DataEngineInstance::get_variant() const {
  ...
  throw DataEngineError(DataEngineErrorNumber::no_variant_found,
                        QString("Dataengine: No dependency fullfilling variant "
                                "found in section: \"%1\"")
                            .arg(section_name));
}

void DataEngineInstance::delete_unmatched_variants(
    const QMap<QString, QList<QVariant>> &tags, uint instance_index,
    uint instance_count) {
  std::vector<VariantData> variants_new;

  for (auto &variant : variants) {
    if (variant.is_dependency_matching(tags, instance_index, instance_count,
                                       section_name)) {
  ...
  if (variants.size() > 1) {
    throw DataEngineError(
        ...
        QString("Dataengine: More than one dependency fullfilling variants "
                "(%1) found in section: \"%2\"")
            .arg(variants.size())
            .arg(section_name));
  }
...
void DataEngineSection::from_json(const QJsonValue &object,
  ...
  if (object.isArray()) {
    for (auto variant : object.toArray()) {
      ...
      if (obj.contains("instance_count")) {
        throw DataEngineError(
            ...
            QString("Dataengine: Instance count of section \"%1\" must not be "
                    "a defined within variant scope.")
      ...
      if (obj.contains("allow_empty_section")) {
        throw DataEngineError(
            ...
            QString("Dataengine: allow_empty_section of section \"%1\" must "
                    "not be a defined within variant scope.")
                .arg(get_section_name()));
      }
    ...
    if (obj.contains("instance_count")) {
      ...
      if ((!is_string) && (!is_number)) {
        throw DataEngineError(
            DataEngineErrorNumber::wrong_type_for_instance_count,
            QString("Dataengine: Instance count of section \"%1\" must not be "
                    "either integer or string. But is \"%2\".")
                .arg(get_section_name())
      ...
      if (is_number) {
        ...
        if (round(instance_count_dbl_json) != instance_count_dbl_json) {
          throw DataEngineError(
              ...
              QString("Dataengine: Instance count of section \"%1\" must not "
                      "be a fraction.")
                  .arg(get_section_name()));
        }
    ...
    if (obj.contains("allow_empty_section")) {
      if (obj["allow_empty_section"].isBool()) {
        prototype_instance.set_allow_empty_section(
            obj["allow_empty_section"].toBool());
      } else {
        throw DataEngineError(
            DataEngineErrorNumber::allow_empty_section_with_wrong_type,
            QString("Dataengine: \"Allow_empty_section\" -tag of section "
                    "\"%1\" must be boolean. But is not.")
                .arg(get_section_name()));
      }
    }
    ...
    if (obj.contains("variants")) {
      ...
      if (var_val.isArray()) {
        ...
        throw std::runtime_error(
            QString("Dataengine: invalid variant in json file").toStdString());
      }
    } else {
...
QString DataEngineSection::get_section_name() const { return section_name; }
...
QStringList DataEngineSection::get_all_ids_of_selected_instance(
...
bool DataEngineSection::set_instance_count_if_name_matches(
    QString instance_count_name, uint instance_count) {
  bool result = false;
  ...
  if (this->instance_count_name == instance_count_name) {
    if (is_section_instance_defined()) {
      throw DataEngineError(
          ...
          QString("Dataengine: Instance count of section \"%1\" is already "
                  "defined. Instance count name is: \"%2\", actual value "
    ...
    if (instance_count.value() == 0) {
      throw DataEngineError(
          DataEngineErrorNumber::
              instance_count_must_not_be_zero_nor_fraction_nor_negative,
          QString(
              "Dataengine: Instance count of section \"%1\" must not be zero.")
              .arg(get_section_name()));
    }
...
void DataEngineSection::assert_instance_is_defined() const {
  if (is_section_instance_defined() == false) {
    throw DataEngineError(
        ...
        QString("Dataengine: Instance count of section \"%1\" yet undefined. "
                "Instance count value is: \"%2\".")
            .arg(get_section_name())
            .arg(get_instance_count_name()));
  }
}

void DataEngineSection::use_instance(QString instance_caption,
                                     uint instance_index) {
  assert_instance_is_defined();
  instance_index--;
  if (instance_index >= instance_count.value()) {
    throw DataEngineError(DataEngineErrorNumber::instance_count_exceeding,
                          QString("Dataengine: Instance index (%1) of section "
                                  "\"%2\" exceeds instance count(%3).")
                              .arg(instance_index)
                              .arg(get_section_name())
...
VariantData::VariantData(const VariantData &other)
    : dependency_tags{other.dependency_tags}, relevant_dependencies{
                                                  other.relevant_dependencies} {
  for (auto &entry : other.data_entries) {
    ...
    if (entry_num) {
      ...
      data_entries.push_back(
          std::make_unique<DateTimeDataEntry>(*entry_dateime));
    } else if (entry_ref) {
...
uint VariantData::get_entry_count() const { return data_entries.size(); }
...
bool VariantData::is_dependency_matching(
    const QMap<QString, QList<QVariant>> &tags, uint instance_index,
    uint instance_count, const QString &section_name) {
  ...
  if (keys_to_test.count()) {
        ...
        if ((bool)length_of_first_tag_value_list == false) {
          ...
          if (value_count != length_of_first_tag_value_list.value()) {
            throw DataEngineError(
                ...
                QString("Dataengine: In section \"%1\": The length of the "
                        "tables of corresponding dependency tag values must be "
    ...
    if ((bool)length_of_first_tag_value_list == false) {
      return false; // if there were no tags matching, it is quite obvious that
                    // the result is false
    }

    if ((instance_count != length_of_first_tag_value_list.value()) &&
        (length_of_first_tag_value_list.value() != 1)) {
      throw DataEngineError(
          DataEngineErrorNumber::
              instance_count_must_match_list_of_dependency_values,
          QString(
              "Dataengine: Instance count of section \"%1\" must have the same "
              "value as the length of the tables of corresponding dependency "
...
void VariantData::from_json(const QJsonObject &object) {
  ...
  if (data.isUndefined() || data.isNull()) {
    throw DataEngineError(
        DataEngineErrorNumber::no_data_section_found,
        QString("Dataengine: no data found ininvalid variant in json file"));
  }
  ...
  if (data.isArray()) {
    if (data.toArray().count() == 0) {
      throw DataEngineError(
          DataEngineErrorNumber::no_data_section_found,
          QString("Dataengine: no data found in variant in json file"));
    }
    for (const auto &data_entry : data.toArray()) {
      std::unique_ptr<DataEngineDataEntry> entry =
          DataEngineDataEntry::from_json(data_entry.toObject());
      if (entry_exists(entry.get()->field_name)) {
        throw DataEngineError(
            DataEngineErrorNumber::duplicate_field,
            QString(
                "Dataengine: Data field with the name %1 is already existing")
                .arg(entry.get()->field_name));
      }
      data_entries.push_back(std::move(entry));
    }
  } else if (data.isObject()) {
    std::unique_ptr<DataEngineDataEntry> entry =
        DataEngineDataEntry::from_json(data.toObject());
    if (entry_exists(entry.get()->field_name)) {
      throw DataEngineError(
          DataEngineErrorNumber::duplicate_field,
          QString("Dataengine: Data field with the name %1 is already existing")
              .arg(entry.get()->field_name));
    }
    data_entries.push_back(std::move(entry));
  }
...
DataEngineDataEntry *VariantData::get_entry(QString field_name) const {
  ...
  if (errornum == DataEngineErrorNumber::no_field_id_found) {
    ...
    assert(0);
    throw DataEngineError(
        errornum, QString("Dataengine: UnkownError at \"%1\"").arg(field_name));
  }
...
DataEngineDataEntry *
VariantData::get_entry_raw(QString field_name,
  ...
  if (object.isObject()) {
    for (auto key : object.toObject().keys()) {
      ...
      if (val.isBool() || val.isDouble() || val.isString()) {
        ...
        throw std::runtime_error(
            QString("Dataengine: invalid type of tag description in json file")
                .toStdString());
      }
    }
    ...
    throw std::runtime_error(
        QString("Dataengine: invalid tag description in json file")
            .toStdString());
  }
}
...
void DependencyValue::from_json(const QJsonValue &object,
                                const bool default_to_match_all) {
  if (object.isString()) {
    ...
    throw std::runtime_error(
        QString("Dataengine: invalid version dependency in json file")
            .toStdString());
  }
}
...
bool DependencyValue::is_matching(const QVariant &test_value) const {
  ...
  case MatchExactly: {
    bool result = true;
    if ((test_value.type() == QVariant::Double) &&
        (match_exactly.type() == QVariant::Double)) {
      bool ok_a, ok_b;
      double num_val_a = test_value.toDouble(&ok_a);
      double num_val_b = match_exactly.toDouble(&ok_b);
      if (std::round(num_val_a * PRECISION_REDUCTION) ==
          std::round(num_val_b * PRECISION_REDUCTION)) {
        result = true;
      }
    } else if ((test_value.type() == QVariant::Bool) &&
               (match_exactly.type() == QVariant::Bool)) {
  ...
  case MatchByRange: {
    ...
    double val_high_excl =
        std::round(range_high_excluding * PRECISION_REDUCTION);
...
void DependencyValue::parse_number(const QString &str, float &vnumber,
  ...
  if (str.trimmed() == "*") {
    ...
    if (!ok) {
      throw DataEngineError(
          DataEngineErrorNumber::invalid_version_dependency_string,
          QString("Dataengine: could not parse dependency string \"%1\" in as "
                  "range number")
              .arg(str));
    }
  }
...
void DependencyValue::from_string(const QString &str) {
  ...
  if (value.startsWith("[") && value.endsWith("]")) {
    ...
    if ((sl.count() == 0) || (value == "")) {
      ...
      throw DataEngineError(
          DataEngineErrorNumber::invalid_version_dependency_string,
          QString("Dataengine: invalid version dependency string \"%1\"in json "
                  "file")
              .arg(str));
    }
  } else {
...
bool Data_engine::value_in_range(const FormID &id) const {
  assert_not_in_dummy_mode();
  QList<const DataEngineDataEntry *> data_entry =
      sections.get_entries_const(id, false);
...
void DataEngineSection::set_actual_number(const FormID &id, double number) {
  ...
  assert(data_entry); // if field is not found an exception was already thrown
                      // above. Something bad must happen to assert here
  ...
  if (!number_entry) {
    if (!reference_entry) {
      throw DataEngineError(
          ...
          QString("Dataengine: The field \"%1\" is not numerical as it must be "
                  "if you set it with the number (%2) ")
              .arg(id)
              .arg(number));
    }
...
void DataEngineSection::set_actual_text(const FormID &id, QString text) {
  ...
  if (!text_entry) {
    if (!reference_entry) {
      throw DataEngineError(
          DataEngineErrorNumber::setting_desired_value_with_wrong_type,
          QString("Dataengine: The field \"%1\" is not a string as it must be "
                  "if you set it with the string \"%2\"")
              .arg(id)
              .arg(text));
    }
...
void DataEngineSection::set_actual_bool(const FormID &id, bool value) {
  ...
  if (!bool_entry) {
    if (!reference_entry) {
      throw DataEngineError(
          DataEngineErrorNumber::setting_desired_value_with_wrong_type,
          QString("Dataengine: The field \"%1\" is not boolean as it must be "
                  "if you set it with a bool type")
              .arg(id));
    }
...
void DataEngineSection::set_actual_datetime(const FormID &id,
                                            DataEngineDateTime value) {
  DataEngineDataEntry *data_entry = get_entry(id);
  assert(data_entry); // if field is not found an exception was already thrown
                      // above. Something bad must happen to assert here
  ...
  if (!value.isValid()) {
    throw DataEngineError(
        DataEngineErrorNumber::datetime_is_not_valid,
        QString("Dataengine: Date time value for the field \"%1\" is not "
                "valid. function: set_actual_datetime")
            .arg(id));
  }
  ...
  if (!date_time_entry) {
    if (!reference_entry) {
      throw DataEngineError(
          DataEngineErrorNumber::setting_desired_value_with_wrong_type,
          QString("Dataengine: The field \"%1\" is not dateime as it must be "
                  "if you set it with a datetime type")
              .arg(id));
    }
    reference_entry->set_actual_value(value);
  } else {
...
QString DataEngineSection::get_section_title() const { return section_title; }
...
void Data_engine::set_actual_datetime(const FormID &id,
                                      DataEngineDateTime value) {
  auto section = sections.get_section(id);
  section->set_actual_datetime(id, value);
}

void Data_engine::use_instance(const QString &section_name,
                               const QString &instance_caption,
                               const uint instance_index) {
  sections.use_instance(section_name, instance_caption, instance_index);
}

void Data_engine::set_instance_count(QString instance_count_name,
                                     uint instance_count) {
  sections.set_instance_count(instance_count_name, instance_count);
}

QStringList
Data_engine::get_instance_captions(const QString &section_name) const {
  DataEngineSection *the_section =
      sections.get_section(section_name + "/dummy");
  assert(the_section);
...
bool Data_engine::section_uses_instances(QString section_name) const {
  DataEngineSection *the_section =
      sections.get_section(section_name + "/dummy");
  assert(the_section);
  return the_section->get_instance_count_name().size() ||
         the_section->get_instance_captions().count() > 1;
}
...
void Data_engine::fill_engine_with_dummy_data() {
  ...
  for (const auto &sectionname : section_names) {
    for (uint instance_i = 1;
         instance_i <= get_instance_count(sectionname.toStdString());
         instance_i++) {
      use_instance(sectionname,
                   "Instance-caption #" + QString::number(instance_i),
                   instance_i);
      ...
      for (const auto &id : ids) {
        if (is_bool(id)) {
          ...
          set_actual_datetime(id,
                              DataEngineDateTime{QDateTime::currentDateTime()});
        } else {
          assert(0);
        }
      }
...
Sol_table Data_engine::get_ids_of_section(sol::state *lua,
...
static QString get_caption(const QString &section_name,
                           const QString &instance_caption) {
  (void)section_name;
...
void Data_engine::save_to_json(QString filename) {
  ...
  if (filename == "") {
    throw DataEngineError(
        DataEngineErrorNumber::cannot_open_file,
        QString{"Dataengine: Filename for dumping data_engine is empty"});
  }

  if (!saveFile.open(QIODevice::WriteOnly)) {
    throw DataEngineError{DataEngineErrorNumber::cannot_open_file,
                          QString{"Dataengine: Can not open file: \"%1\" for "
                                  "dumping data_engine content."}
                              .arg(filename)};
  }
  ...
  auto script_git =
      git_info(QFileInfo(script_path).absolutePath(), true, false);
  jo_general["datetime_unix"] = QDateTime::currentMSecsSinceEpoch() / 1000;
  jo_general["datetime_str"] =
  ...
  QVariant duration{};
  duration.setValue<qint64>(QDateTime::currentMSecsSinceEpoch() / 1000 -
                            load_time_seconds_since_epoch);
  ...
  const QMap<QString, QList<QVariant>> dependency_tags =
      sections.get_dependancy_tags();
  ...
  for (const DataEngineSection &section : sections.sections) {
    ...
    for (const DataEngineInstance &instance : section.instances) {
      ...
      auto jo_relevant_dependencies =
          QJsonObject::fromVariantMap(relevant_dependencies);

      for (const std::unique_ptr<DataEngineDataEntry> &entry :
           variant->data_entries) {
        ...
        if (entry->get_exceptional_approval().approved) {
          jo_field["exceptional_approval"] =
              entry->get_exceptional_approval().get_json_dump();
          exceptional_approval_exists = true;
        }
        jo_field[entry->get_specific_json_name()] =
            entry->get_specific_json_dump();
...
void Data_engine::fill_database(QSqlDatabase &db) const {
  ...
  for (const DataEngineSection &section : sections.sections) {
    ...
    for (const DataEngineInstance &instance : section.instances) {
      db_exec(db,
              QString{"INSERT INTO %1 VALUES(%2, '%3')"}.arg(
                  instances_table_name, QString::number(instance_id_counter),
      ...
      for (const std::unique_ptr<DataEngineDataEntry> &entry :
        ...
        if (ea.approved) {
          failed_string +=
              "(" + QString::number(ea.exceptional_approval.id) + ")";
          if (!entry->is_in_range()) {
            bool found = false;
            for (const auto &ea_item :
                 exceptional_approvals) { // grouping approvals
              if (ea_item.exceptional_approval.id ==
                  ea.exceptional_approval.id) {
                found = true;
                break;
              }
            }
        ...
        db_exec(
            ...
            QString{"INSERT INTO %1 VALUES(%2, '%3', '%4', '%5', '%6', %7, "
                    "'%8', '%9')"}
                .arg(section_table_name, QString::number(id),
                     section.get_section_name() + "/" + entry->field_name,
                     entry->get_description(),
                     entry->get_desired_value_as_string(),
                     entry->get_actual_values(),
                     QString::number(instance_id_counter),
                     entry->is_in_range() ? QObject::tr("Ok") : failed_string,
                     entry->get_unit()));
        id++;
      }
  ...
  for (const auto &ea : exceptional_approvals) {
    db_exec(db, QString{"INSERT INTO %1 VALUES(%2, '%3', '%4')"}.arg(
                    exceptional_approvals_table_name,
                    QString::number(ea.exceptional_approval.id),
                    ea.exceptional_approval.description,
                    ea.approving_operator_name));
  }
}

void Data_engine::do_exceptional_approval_(ExceptionalApprovalDB &ea_db,
...
static FailedField
failed_field_from_data_entry(const DataEngineDataEntry *entry, QString field_id,
                             QString instance_caption) {
...
bool Data_engine::do_exceptional_approval(ExceptionalApprovalDB &ea_db,
                                          QString field_id, QWidget *parent) {
  QList<FailedField> failed_fields;

  auto entry = sections.get_actual_instance_entry_const(field_id);
  if (((entry->is_in_range() == false) || (entry->is_complete() == false)) ==
      true) {
    auto section = sections.get_section(field_id);
    assert(section);
    auto instance_caption = section->get_actual_instance_caption();
    FailedField failed_field =
        failed_field_from_data_entry(entry, field_id, instance_caption);
...
void Data_engine::do_exceptional_approvals(ExceptionalApprovalDB &ea_db,
                                           QWidget *parent) {
  QList<FailedField> failed_fields;
  int instance_id_counter = 1;
  for (const DataEngineSection &section : sections.sections) {
    for (const DataEngineInstance &instance : section.instances) {
      ...
      for (const std::unique_ptr<DataEngineDataEntry> &entry :
           variant->data_entries) {
        if (((entry->is_in_range() == false) ||
             (entry->is_complete() == false)) == true) {
          FailedField failed_field = failed_field_from_data_entry(
              entry.get(), section.get_section_name() + "/" + entry->field_name,
              instance.instance_caption);
          failed_fields.append(failed_field);
        }
      }
...
struct XML {
  XML(const QString &name) { state.xml.writeStartElement(name); }
  XML &attribute(const QString &name, const QString &value) {
    state.xml.writeAttribute(name, value);
    return *this;
  }
  XML &
  attributes(const std::initializer_list<std::pair<QString, QString>> &list) {
    for (auto &item : list) {
  ...
  ~XML() { state.xml.writeEndElement(); }
  XML &add_band_index_element() {
    XML("bandIndex")
        .attributes(
            {{"Type", "int"}, {"Value", QString::number(state.band_index++)}});
    return *this;
  }

  static auto &y() { return state.y; }
...
void Data_engine::generate_template(
    const QString &destination, const QString &db_filename,
    QString report_title, QString image_footer_path, QString image_header_path,
    QString approved_by_field_id, QString static_text_report_header,
    QString static_text_page_header, QString static_text_page_footer,
    QString static_text_report_footer_above_signature,
    QString static_text_report_footer_beneath_signature,
  ...
  xml.writeStartElement("Report");
  {
    ...
    xml.writeAttribute("ClassName", "LimeReport::ReportEnginePrivate");
    {
      ...
      generate_pages(xml, report_title, image_footer_path, image_header_path,
                     approved_by_field_id, static_text_report_header,
                     static_text_page_header, static_text_page_footer,
                     static_text_report_footer_above_signature,
...
void Data_engine::generate_pages(
    QXmlStreamWriter &xml, QString report_title, QString image_footer_path,
    QString image_header_path, QString approved_by_field_id,
    QString static_text_report_header, QString static_text_page_header,
    QString static_text_page_footer,
    QString static_text_report_footer_above_signature,
    QString static_text_report_footer_beneath_signature,
    const QList<PrintOrderItem> &print_order) const {
  XML pages{"item"};
  pages.attributes(
      {{"Type", "Object"}, {"ClassName", "LimeReport::PageDesignIntf"}});
  {
    ...
    XML{"objectName"}.attribute("Type", "QString").value("ReportPage");
    {
      ...
      generate_pages_header(
          xml, report_title, image_footer_path, image_header_path,
          approved_by_field_id, static_text_report_header,
          static_text_page_header, static_text_page_footer,
          static_text_report_footer_above_signature,
          static_text_report_footer_beneath_signature, print_order);
...
int Data_engine::generate_image(QXmlStreamWriter &xml, QString image_path,
                                int y_position, QString parent_name) const {
  ...
  if (image_valid) {
    ...
    XML{"geometry"}.attributes(
        {{"width", QString::number(desired_width)},
...
int Data_engine::generate_textfields(
    QXmlStreamWriter &xml, int y_start,
    const QList<PrintOrderItem> &print_order,
    TextFieldDataBandPlace actual_band_position) const {
  int y = y_start;
  int text_field_height = 50;
  int font_size = 10;
  QString font_color = "#000000";
  if ((actual_band_position == TextFieldDataBandPlace::page_footer) ||
      (actual_band_position == TextFieldDataBandPlace::page_header)) {
    text_field_height = 30;
    font_size = 6;
    font_color = "#a8a8a8";
  }

  QList<PrintOrderSectionItem> textfield_print_order =
      get_print_order(print_order, true, actual_band_position);
  for (auto section_order : textfield_print_order) {
    ...
    assert(variant);
    const int text_field_width =
        2000 / section_order.print_order_item.text_field_column_count;
    unsigned int col = 0;
    for (const auto &fields : variant->data_entries) {
      ...
      xml.writeStartElement("item");
      {
        ...
        XML{"geometry"}.attributes(
            {{"width", QString::number(text_field_width)},
        ...
        XML{"font"}.attributes({{"pointSize", QString::number(font_size)},
                                ...
                                {"bold", "0"},
                                {"underline", "0"}});
        XML{"fontColor"}.attributes(
            {{"Value", font_color}, {"Type", "QColor"}});

        XML{"alignment"}.attributes(
            {{"Value", "129"}, {"Type", "enumAndFlags"}});
        XML{"allowHTML"}.attributes({{"Value", "1"}, {"Type", "bool"}});
        XML{"autoHeight"}.attributes({{"Value", "1"}, {"Type", "bool"}});
        XML{"backgroundMode"}.attributes(
            {{"Value", "0"}, {"Type", "enumAndFlags"}});
        XML{"content"}
            .attribute("Type", "QString")
            .value("<b>" + field->get_description() + "</b>: $D{" +
                   section_order.section->get_sql_section_name() + "_" +
                   field->field_name + ".Actual}");
...
int Data_engine::generate_static_text_field(
    QXmlStreamWriter &xml, int y_start, const QString static_text,
    TextFieldDataBandPlace actual_band_position) const {
  int y = y_start;
  int text_field_height = 50;
  int font_size = 10;
...
void Data_engine::generate_pages_header(
    QXmlStreamWriter &xml, QString report_title, QString image_footer_path,
    QString image_header_path, QString approved_by_field_id,
    QString static_text_report_header, QString static_text_page_header,
    QString static_text_page_footer,
    ...
    const QList<PrintOrderItem> &print_order) const {
  {
    ...
    {
      report_header.attributes(
          {{"Type", "Object"}, {"ClassName", "ReportHeader"}});
      ...
      xml.writeStartElement("children");
      {
        ...
        xml.writeStartElement("item");
        {
          ...
          XML{"geometry"}.attributes({{"width", "2000"},
                                      {"height", QString::number(y_position)},
                                      {"x", "0"},
                                      {"y", "0"},
                                      {"Type", "QRect"}});
        ...
        xml.writeEndElement(); // item
        y_position += 100;
        y_position = generate_textfields(xml, y_position, print_order,
                                         TextFieldDataBandPlace::report_header);

        y_position = generate_static_text_field(
            xml, y_position, static_text_report_header,
            TextFieldDataBandPlace::report_header);
      }
  ...
  }
  {
    ...
    {
      int y_position = 0;
      paget_Header.attributes(
          {{"Type", "Object"}, {"ClassName", "PageHeader"}});
      ...
      { //
        xml.writeAttribute("Type", "Collection");
        y_position =
            generate_image(xml, image_header_path, y_position, header_name);
        y_position = generate_textfields(xml, y_position, print_order,
                                         TextFieldDataBandPlace::page_header);
        y_position =
            generate_static_text_field(xml, y_position, static_text_page_header,
                                       TextFieldDataBandPlace::page_header);
      }
      xml.writeEndElement(); // children
      XML{"geometry"}.attributes({{"height", QString::number(y_position + 30)},
                                  {"Type", "QRect"},
                                  {"width", "2000"}});
      XML{"autoHeight"}.attributes({{"Value", "1"}, {"Type", "bool"}});
    }
  }
  {
    XML paget_footer{"item"};
    {
      ...
      paget_footer.attributes(
          {{"Type", "Object"}, {"ClassName", "PageFooter"}});
      ...
      xml.writeStartElement("children");
      {
        ...
        y_position =
            generate_static_text_field(xml, y_position, static_text_page_footer,
                                       TextFieldDataBandPlace::page_footer);
        ...
        xml.writeStartElement("item");
        {
          xml.writeAttribute("ClassName", "TextItem");
          {
            ...
            XML{"font"}.attributes({{"pointSize", "10"},
                                    ...
                                    {"underline", "0"}});
            XML{"alignment"}.attributes(
                {{"Value", "36"}, {"Type", "enumAndFlags"}});
            XML{"content"}
                .attribute("Type", "QString")
                .value("$V{#PAGE}/$V{#PAGE_COUNT}");
            XML{"backgroundMode"}.attributes(
                {{"Value", "0"}, {"Type", "enumAndFlags"}});
            xml.writeStartElement("objectName");
            {
        ...
        y_position =
            generate_image(xml, image_footer_path, y_position, footer_name);
      }
      xml.writeEndElement(); // children
      XML{"geometry"}.attributes({{"height", QString::number(y_position)},
                                  {"Type", "QRect"},
                                  {"width", "2000"}});
      XML{"autoHeight"}.attributes({{"Value", "1"}, {"Type", "bool"}});
    }
  }
  {
    XML report_header{"item"};
    {
      int y_position = 0;
      report_header.attributes(
          {{"Type", "Object"}, {"ClassName", "ReportFooter"}});
      report_header.add_band_index_element();
      ...
      xml.writeStartElement("children");
      {
        xml.writeAttribute("Type", "Collection");
        y_position = generate_textfields(xml, y_position, print_order,
                                         TextFieldDataBandPlace::report_footer);

        y_position = generate_static_text_field(
            xml, y_position, static_text_report_footer_above_signature,
            TextFieldDataBandPlace::report_footer);
        ...
        xml.writeStartElement("item");
        {
          ...
          {
            XML{"geometry"}.attributes({{"width", "1000"},
                                        {"height", "50"},
                                        {"x", "500"},
                                        {"y", QString::number(y_position)},
                                        {"Type", "QRect"}});
        ...
        xml.writeStartElement("item");
        {
          xml.writeAttribute("ClassName", "TextItem");
          {
            XML{"geometry"}.attributes({{"width", "200"},
                                        {"height", "50"},
                                        {"x", "1300"},
                                        {"y", QString::number(y_position + 25)},
                                        {"Type", "QRect"}});
            XML{"font"}.attributes({{"pointSize", "10"},
                                    {"family", "Arial"},
                                    {"weight", "50"},
                                    ...
                                    {"underline", "0"}});
            XML{"alignment"}.attributes(
                {{"Value", "34"}, {"Type", "enumAndFlags"}});
            XML{"content"}
                .attribute("Type", "QString")
                .value(QObject::tr("Signature"));
            XML{"backgroundMode"}.attributes(
                {{"Value", "0"}, {"Type", "enumAndFlags"}});
            XML{"autoHeight"}.attributes({{"Value", "1"}, {"Type", "bool"}});
            xml.writeStartElement("objectName");
        ...
        auto section_with_approved_by =
            sections.get_section_no_exception(approved_by_field_id);
        QString approved_query = "";
        if (section_with_approved_by) {
          approved_query =
              "$D{" + section_with_approved_by->get_sql_section_name() + "_" +
              adjust_sql_table_name(approved_by_field_id.split("/")[1]) +
              "_automatic.Actual}";
        }
        xml.writeStartElement("item");
        {
          xml.writeAttribute("ClassName", "TextItem");
          {
            XML{"geometry"}.attributes({{"width", "250"},
                                        {"height", "50"},
                                        {"x", "250"},
                                        {"y", QString::number(y_position)},
                                        {"Type", "QRect"}});
        ...
        y_position = generate_static_text_field(
            xml, y_position, static_text_report_footer_beneath_signature,
            TextFieldDataBandPlace::report_footer);
      }
...
QList<PrintOrderSectionItem> Data_engine::get_print_order(
    const QList<PrintOrderItem> &orig_print_order, bool used_for_textfields,
    TextFieldDataBandPlace actual_band_position) const {
  ...
  for (auto order_item : orig_print_order) {
    if (order_item.print_enabled) {
      if ((used_for_textfields || order_item.print_as_text_field) == false) {
        ...
        if ((actual_band_position == order_item.text_field_place) ||
            (actual_band_position == TextFieldDataBandPlace::all)) {
          print_order_items.append(order_item);
        }
      }
    }
  ...
  for (auto print_order_item : print_order_items) {
    DataEngineSection *section =
        sections.get_section(print_order_item.section_name + "/dummy");
    assert(section);
...
void Data_engine::generate_tables(
    const QList<PrintOrderItem> &print_order) const {
  auto section_print_order =
      get_print_order(print_order, false, TextFieldDataBandPlace::none);
...
void Data_engine::generate_exception_approval_table() const {
  const auto &headers = {QObject::tr("ID"), QObject::tr("Description"),
                         QObject::tr("Approved by"), QObject::tr("Signature")};
  const int column_widths[4] = {5, 40, 25, 30};
  const auto &sql_fields = {"ID", "Description", "approving_person", ""};
  // Data Band Header
  {
    XML data_band_header{"item"};
    data_band_header.attributes(
        {{"Type", "Object"}, {"ClassName", "DataHeader"}});
    data_band_header.add_band_index_element();
    XML{"parentBand"}
        .attribute("Type", "QString")
        .value(QString{"DataBand%1"}.arg(exceptional_approvals_table_name));
    XML{"geometry"}.attributes({{"width", "2000"},
                                {"height", "170"},
                                {"x", "0"},
                                {"y", QString::number(XML::y() += 100)},
                                {"Type", "QRect"}});
    XML{"objectName"}
        .attribute("Type", "QString")
        .value(QString{"DataBandHeader_exceptional_approval"});
      ...
      {
        {
          ...
          XML{"content"}
              .attribute("Type", "QString")
              .value(QObject::tr("Exceptional Approvals"));
          XML{"objectName"}
              .attribute("Type", "QString")
              .value(QString{"DataHeaderTitle_exceptional_approval"});
        }
        int x_position = 0;
        for (std::size_t i = 0; i < headers.size(); i++) {
          ...
          XML{"objectName"}
              .attribute("Type", "QString")
              .value(
                  QString{"DataHeaderColumnHeader_%1_exceptional_approval"}.arg(
                      header));
  ...
  // Data Band
  {
    ...
    XML{"datasource"}
        .attribute("Type", "QString")
        .value(exceptional_approvals_table_name);
    XML{"splittable"}.attributes({{"Value", "0"}, {"Type", "bool"}});
...

    {
      ...
      const int row_height = 70;
      {
        ...
        for (std::size_t i = 0; i < headers.size() - 1; i++) {
          ...
          XML{"content"}
              .attribute("Type", "QString")
              .value(QString{"$D{%1.%2}"}.arg(exceptional_approvals_table_name,
                                              sql_fields.begin()[i]));
          XML{"objectName"}
              .attribute("Type", "QString")
              .value(QString{} + "DataHeaderColumn" + header +
                     exceptional_approvals_table_name);
          x_position += field_width;
        }
        const int field_width = 2000 * column_widths[3] / 100;
        {
          XML item{"item"};
          {
            ...
            {
              XML{"geometry"}.attributes(
                  {{"width", QString::number(field_width)},
                   {"height", "-1"},
                   {"x", QString::number(x_position)},
  ...
  // Data Band Footer
  {
    ...
    XML{"objectName"}
        .attribute("Type", "QString")
        .value(QString{"DataFooter%1"}.arg(exceptional_approvals_table_name));
    XML{"splittable"}.attributes({{"Value", "0"}, {"Type", "bool"}});
...
void Data_engine::generate_table(const DataEngineSection *section) const {
  const auto &headers = {QObject::tr("Name"), QObject::tr("Target Value"),
                         QObject::tr("Actual Value"), QObject::tr("Result")};
  ...
  // Data Band Header
  {
    ...
    XML{"parentBand"}
        .attribute("Type", "QString")
        .value(QString{"DataBand%1"}.arg(section->get_sql_section_name()));
    XML{"geometry"}.attributes({{"width", "2000"},
                                {"height", "170"},
                                {"x", "0"},
                                {"y", QString::number(XML::y() += 100)},
    ...
    XML{"objectName"}
        .attribute("Type", "QString")
        .value(
            QString{"DataBandHeader_%1"}.arg(section->get_sql_section_name()));
      ...
      {
        {
          ...
          XML{"content"}
              .attribute("Type", "QString")
              .value(section->get_section_title());
          XML{"objectName"}
              .attribute("Type", "QString")
              .value(QString{"DataHeaderTitle_%1"}.arg(
                  section->get_sql_section_name()));
        }
        int x_position = 0;
        for (std::size_t i = 0; i < headers.size(); i++) {
          const int field_width = 2000 * column_widths[i] / 100;
          ...
          XML{"objectName"}
              .attribute("Type", "QString")
              .value(QString{"DataHeaderColumnHeader_%1_%2"}.arg(
                  header, section->get_sql_instance_name()));
          x_position += field_width;
        }
      }
    }
  ...
  // Data Band
  {
    ...
    XML{"datasource"}
        .attribute("Type", "QString")
        .value(section->get_sql_instance_name());
    XML{"splittable"}.attributes({{"Value", "0"}, {"Type", "bool"}});
    XML{"keepBottomSpace"}.attributes({{"Value", "0"}, {"Type", "bool"}});
    ...
    XML{"keepSubdetailTogether"}.attributes({{"Value", "0"}, {"Type", "bool"}});
    {
      ...
      children.attribute("Type", "Collection");
      {
        ...
        XML{"content"}
            .attribute("Type", "QString")
            .value(QString{"$D{%1.%2}"}.arg(section->get_sql_instance_name(),
                                            "Caption"));
        XML{"objectName"}
            .attribute("Type", "QString")
            .value("DataSectionName_" + section->get_sql_section_name());
      }
    }
  }
  // Data Band Subdetail
  {
    ...
    data_subdetail_header.attributes(
        {{"Type", "Object"}, {"ClassName", "SubDetail"}});
    ...
    XML{"objectName"}
        .attribute("Type", "QString")
        .value(QString{"SubDetail%1"}.arg(section->get_sql_section_name()));
    XML{"datasource"}
        .attribute("Type", "QString")
        .value(section->get_sql_section_name());
    XML{"splittable"}.attributes({{"Value", "0"}, {"Type", "bool"}});
    XML{"alternateBackgroundColor"}.attributes(
        {{"Value", "#F2F2F2"}, {"Type", "QColor"}});
    XML{"keepBottomSpace"}.attributes({{"Value", "0"}, {"Type", "bool"}});
    XML{"keepFooterTogether"}.attributes({{"Value", "0"}, {"Type", "bool"}});

    {
      ...
      for (std::size_t i = 0; i < headers.size(); i++) {
        ...
        XML{"content"}
            .attribute("Type", "QString")
            .value(QString{"$D{%1.%2}"}.arg(section->get_sql_section_name(),
                                            sql_fields.begin()[i]));
        XML{"objectName"}
            .attribute("Type", "QString")
            .value(QString{} + "DataHeaderColumn" + header +
  ...
  {
    XML data_footer{"item"};
    data_footer.attributes(
        {{"Type", "Object"}, {"ClassName", "SubDetailFooter"}});
    data_footer.add_band_index_element();
    XML{"parentBand"}
        .attribute("Type", "QString")
        .value(QString{"SubDetail%1"}.arg(section->get_sql_section_name()));
    ...
    XML{"objectName"}
        .attribute("Type", "QString")
        .value(
            QString{"SubDetailFooter%1"}.arg(section->get_sql_section_name()));
    XML{"splittable"}.attributes({{"Value", "0"}, {"Type", "bool"}});
    XML{"keepBottomSpace"}.attributes({{"Value", "1"}, {"Type", "bool"}});
  ...
  // Data Band Footer
  {
    ...
    XML{"objectName"}
        .attribute("Type", "QString")
        .value(QString{"DataFooter%1"}.arg(section->get_sql_section_name()));
    XML{"splittable"}.attributes({{"Value", "0"}, {"Type", "bool"}});
    XML{"keepBottomSpace"}.attributes({{"Value", "1"}, {"Type", "bool"}});
...
void Data_engine::replace_database_filename(
    const std::string &source_form_path,
    const std::string &destination_form_path,
  ...
  xml_file_out.open(QFile::OpenModeFlag::WriteOnly |
                    QFile::OpenModeFlag::Truncate);
  ...
  while ((!xml_in.atEnd())) {
    ...
    if (xml_in.isStartElement()) {
      if (xml_in.name() == "databaseName") {
        XML{"databaseName"}
            .attribute("Type", "QString")
            .value(QString::fromStdString(database_path));
...
void Data_engine::add_sources_to_form(QString data_base_path,
                                      const QList<PrintOrderItem> &print_order,
                                      QString approved_by_field_id) const {
  ...
  const auto connection_name = "default_connection";
  {
    ...
    XML item{"item"};
    item.attributes(
        {{"ClassName", "LimeReport::ConnectionDesc"}, {"Type", "Object"}});
  ...
  // queries
  {
    ...
    for (const auto &section : sections.sections) {
      XML item{"item"};
      item.attributes(
          {{"ClassName", "LimeReport::QueryDesc"}, {"Type", "Object"}});
      XML{"queryName"}
          .attribute("Type", "QString")
          .value(section.get_sql_instance_name());
      XML{"queryText"}
          .attribute("Type", "QString")
          .value(
              QString{"SELECT * FROM %1"}.arg(section.get_sql_instance_name()));
      XML{"connectionName"}.attribute("Type", "QString").value(connection_name);
    }

    QList<PrintOrderSectionItem> textfield_print_order =
        get_print_order(print_order, true, TextFieldDataBandPlace::all);

    PrintOrderSectionItem approved_id;
    DataEngineSection *approved_section =
        sections.get_section_no_exception(approved_by_field_id);
    if (approved_section) {
      approved_id.section = approved_section;
      approved_id.field_name =
          adjust_sql_table_name(approved_by_field_id.split("/")[1]);
    ...
    for (const auto &section_item : textfield_print_order) {
      ...
      for (auto &field_name : field_names) {
        ...
        XML{"queryName"}
            .attribute("Type", "QString")
            .value(section_item.section->get_sql_section_name() + "_" +
                   field_name + section_item.suffix);
        XML{"queryText"}
            .attribute("Type", "QString")
            .value(QString{"SELECT * FROM %1 WHERE Name = \"%2\""}
                       .arg(section_item.section->get_sql_section_name())
                       .arg(section_item.section->get_section_name() + "/" +
                            field_name));
        XML{"connectionName"}
            .attribute("Type", "QString")
    ...
    }
    {
      ...
      XML{"queryText"}
          .attribute("Type", "QString")
          .value(QString{"SELECT * FROM %1 ORDER BY ID"}.arg(
              exceptional_approvals_table_name));
      XML{"connectionName"}.attribute("Type", "QString").value(connection_name);
    }
  }
  ...
  // subqueries
  {
    ...
    for (const auto &section : sections.sections) {
      XML item{"item"};
      item.attributes(
          {{"ClassName", "LimeReport::SubQueryDesc"}, {"Type", "Object"}});
      XML{"queryName"}
          .attribute("Type", "QString")
          .value(section.get_sql_section_name());
      XML{"queryText"}
          .attribute("Type", "QString")
          .value(
              QString{"SELECT * FROM %1 WHERE InstanceID = $D{%2.InstanceID}"}
                  .arg(section.get_sql_section_name(),
                       section.get_sql_instance_name()));
      XML{"connectionName"}.attribute("Type", "QString").value(connection_name);
      XML{"master"}
          .attribute("Type", "QString")
          .value(section.get_sql_instance_name());
...
void Data_engine::assert_not_in_dummy_mode() const {
  if (sections.is_dummy_data_mode) {
    throw DataEngineError(DataEngineErrorNumber::is_in_dummy_mode,
                          "Dataengine: Dataengine is in dummy mode. Access to "
                          "data functions is prohibited.");
  }
}
...
void Data_engine::assert_in_dummy_mode() const {
  if (sections.is_dummy_data_mode == false) {
    throw DataEngineError(DataEngineErrorNumber::dummy_mode_necessary,
                          "Dataengine: Dataengine is not in dummy mode while "
                          "access through dummy mode function.");
  }
}

bool Data_engine::entry_compare(const FormIdWrapper &lhs,
                                const FormIdWrapper &rhs) {
  return lhs.value < rhs.value;
}

double
NumericTolerance::get_absolute_limit_beneath(const double desired) const {
  ...
  case ToleranceType::Percent: {
    double deviation_limit_abs =
        std::abs(desired) * deviation_limit_beneath / 100.0;
...
bool NumericTolerance::test_in_range(
  ...
  if (is_undefined) {
    throw DataEngineError(
        ...
        "Dataengine: no tolerance defined even though a range check should be "
        "done.");
  }
  if (!measured) {
    return false;
  ...
  if (std::isnan(measured.value())) { // fixes #1113 Fehler: "Dataengine: NaN
                                      // ist immer OK, soll natürlich nicht"
    return false;
  }
...
void NumericTolerance::from_string(const QString &str) {
  ...
  if (sl.count() == 1) {
    ...
    if (open_range_a || open_range_b) {
      ...
      throw DataEngineError(DataEngineErrorNumber::tolerance_parsing_error,
                            "Dataengine: tolerance + and - must of the same "
                            "type(either absolute or percent)");
    }
    ...
    throw DataEngineError(DataEngineErrorNumber::tolerance_parsing_error,
                          "Dataengine: tolerance string faulty");
  }
}
...
QString NumericTolerance::to_string(const double desired_value) const {
  ...
  if (open_range_above && open_range_beneath) {
    ...
    if (deviation_limit_beneath > 0.0) {
      result +=
          " (-" + num_to_str(deviation_limit_beneath, tolerance_type) + ")";
    }
...
void NumericTolerance::str_to_num(QString str_in, double &number,
                                  ToleranceType &tol_type, bool &open,
                                  const QStringList expected_sign_strings) {
  ...
  if (str_in == "*") {
    ...
    if (!ok) {
      throw DataEngineError(
          DataEngineErrorNumber::tolerance_parsing_error,
          QString("Dataengine: could not convert string to number %1")
              .arg(str_in_orig));
    }
  }
  if (sign_error) {
    throw DataEngineError(
        DataEngineErrorNumber::tolerance_parsing_error,
        QString("Dataengine: tolerance string should have sign flags %1")
            .arg(str_in_orig));
  }
}

QString NumericTolerance::num_to_str(double number,
                                     ToleranceType tol_type) const {
  QString numStr;
...
void DataEngineDataEntry::set_exceptional_approval(
    ExceptionalApprovalResult exceptional_approval) {
  this->exceptional_approval = exceptional_approval;
}

const ExceptionalApprovalResult &
DataEngineDataEntry::get_exceptional_approval() const {
  return exceptional_approval;
}

std::unique_ptr<DataEngineDataEntry>
DataEngineDataEntry::from_json(const QJsonObject &object) {
  FormID field_name;
  ...
  if (!keys.contains("value")) {
    if (!keys.contains("type")) {
      throw DataEngineError(
          DataEngineErrorNumber::data_entry_contains_neither_type_nor_value,
  ...
  if (!keys.contains("name")) {
    throw DataEngineError(DataEngineErrorNumber::data_entry_contains_no_name,
                          "Dataengine: JSON object must contain key \"name\"");
  }
    ...
    for (const auto &key : keys) {
      if (key == "name") {
        ...
        if (object.value(key).isDouble()) {
          ...
          throw DataEngineError(DataEngineErrorNumber::tolerance_parsing_error,
                                "Dataengine: wrong tolerance type");
        }
        ...
        throw DataEngineError(DataEngineErrorNumber::invalid_data_entry_key,
                              "Dataengine: Invalid key \"" + key +
                                  "\" in numeric JSON object");

      } else {
        throw DataEngineError(
            DataEngineErrorNumber::invalid_data_entry_key,
            "Dataengine: Invalid key \"" + key +
                "\" in numeric JSON object without desired value");
      }
    }
    if ((tolerance.is_defined() == false) && (entry_without_value == false)) {
      throw DataEngineError(
          DataEngineErrorNumber::tolerance_must_be_defined_for_numbers,
          "Dataengine: The number field with key \"" + field_name +
              "\" has no tolerance defined. Each number field must have a "
              "tolerance defined.");
    }
    return std::make_unique<NumericDataEntry>(field_name, desired_value,
                                              tolerance, std::move(unit),
                                              si_prefix, std::move(nice_name));
  }
  case EntryType::Bool: {
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wmaybe-uninitialized"
  ...
  case EntryType::DateTime: {
    ...
    for (const auto &key : keys) {
      if (key == "name") {
        ...
        throw DataEngineError(
            DataEngineErrorNumber::datetime_dont_support_desired_values_yet,
            QString("Dataengine: Datetime fields dont support desired values "
                    "yet. Field_name:  %1")
                .arg(key));
      } else if ((key == "type") && (entry_without_value == true)) {
        // already handled above
  ...
  case EntryType::Reference: {
    ...
    for (const auto &key : keys) {
      if (key == "name") {
        ...
        throw DataEngineError(DataEngineErrorNumber::invalid_data_entry_key,
                              "Dataengine: Invalid key \"" + key +
                                  "\" in reference JSON object");
      }
    }
    return std::make_unique<ReferenceDataEntry>(field_name, reference_string,
                                                tolerance, nice_name);
  }
  case EntryType::Unspecified: {
    throw DataEngineError(
        DataEngineErrorNumber::invalid_data_entry_type,
        QString("Dataengine: Invalid type in JSON object. Field name: %1")
            .arg(field_name));
  }
  }
  throw DataEngineError(
      DataEngineErrorNumber::invalid_json_object,
      QString{"Dataengine: Invalid JSON object. Field name: %1"}.arg(
          field_name));
}

NumericDataEntry::NumericDataEntry(const NumericDataEntry &other)
    : DataEngineDataEntry(other.field_name),
      desired_value{other.desired_value}, unit{other.unit},
      description{other.description}, si_prefix{other.si_prefix},
      tolerance{other.tolerance}, actual_value{other.actual_value} {}

NumericDataEntry::NumericDataEntry(
    FormID field_name, std::experimental::optional<double> desired_value,
    NumericTolerance tolerance, QString unit,
    std::experimental::optional<double> si_prefix, QString description)
    : DataEngineDataEntry(field_name), desired_value(desired_value),
      unit(std::move(unit)), description(std::move(description)),
      tolerance(tolerance) {
...
double NumericDataEntry::get_actual_number() const {
  ...
  throw DataEngineError(
      DataEngineErrorNumber::actual_value_not_set,
      QString("Dataengine: Actual value of field %1 not set").arg(field_name));
}
...
QString NumericDataEntry::get_description() const { return description; }

QString NumericDataEntry::get_unit() const { return unit; }

double NumericDataEntry::get_si_prefix() const { return si_prefix; }

bool NumericDataEntry::compare_unit_desired_siprefix(
    const DataEngineDataEntry *from) const {
...
NumericTolerance NumericDataEntry::get_tolerance() const { return tolerance; }
...
QString NumericDataEntry::get_specific_json_name() const { return "numeric"; }

void NumericDataEntry::set_desired_value_from_desired(
    DataEngineDataEntry *from) {
  NumericDataEntry *num_from = from->as<NumericDataEntry>();
  assert(num_from);
  desired_value = num_from->desired_value;
}

void NumericDataEntry::set_desired_value_from_actual(
    DataEngineDataEntry *from) {
  NumericDataEntry *num_from = from->as<NumericDataEntry>();
  assert(num_from);
...
TextDataEntry::TextDataEntry(const TextDataEntry &other)
    : DataEngineDataEntry{other.field_name}, desired_value{other.desired_value},
      description{other.description}, actual_value{other.actual_value} {}

TextDataEntry::TextDataEntry(const FormID name,
                             std::experimental::optional<QString> desired_value,
                             QString description)
    : DataEngineDataEntry(name), desired_value(std::move(desired_value)),
...
double TextDataEntry::get_actual_number() const {
  throw DataEngineError(
      DataEngineErrorNumber::actual_value_is_not_a_number,
      QString("Dataengine: Actual value of field %1 is not a number")
          .arg(field_name));
}

QString TextDataEntry::get_description() const { return description; }
...
QString TextDataEntry::get_unit() const { return ""; }

double TextDataEntry::get_si_prefix() const { return 1; }

double TextDataEntry::get_desired_number() const {
  throw DataEngineError(
      DataEngineErrorNumber::desired_value_is_not_a_number,
      QString("Dataengine: Desired value of field %1 not a number, it is text")
          .arg(field_name));
}

bool TextDataEntry::compare_unit_desired_siprefix(
    const DataEngineDataEntry *from) const {
...
bool TextDataEntry::is_desired_value_set() const { return (bool)desired_value; }
...
QString TextDataEntry::get_specific_json_name() const { return "text"; }

DateTimeDataEntry::DateTimeDataEntry(const DateTimeDataEntry &other)
    : DataEngineDataEntry{other.field_name}, description{other.description},
      actual_value{other.actual_value} {}

DateTimeDataEntry::DateTimeDataEntry(const FormID name, QString description)
    : DataEngineDataEntry(name), description(description) {}
...
double DateTimeDataEntry::get_actual_number() const {
  throw DataEngineError(
      DataEngineErrorNumber::actual_value_is_not_a_number,
      QString("Dataengine: Actual value of field %1 is datetime, not a number")
          .arg(field_name));
}

QString DateTimeDataEntry::get_description() const { return description; }

QString DateTimeDataEntry::get_desired_value_as_string() const { return ""; }

QString DateTimeDataEntry::get_unit() const { return ""; }

double DateTimeDataEntry::get_si_prefix() const { return 1; }

double DateTimeDataEntry::get_desired_number() const {
  throw DataEngineError(
      DataEngineErrorNumber::desired_value_is_not_a_number,
      QString(
          "Dataengine: Desired value of field %1 not a number, it is datetime")
          .arg(field_name));
}

bool DateTimeDataEntry::compare_unit_desired_siprefix(
    const DataEngineDataEntry *from) const {
...
void DateTimeDataEntry::set_desired_value_from_desired(
    DataEngineDataEntry *from) {
  (void)from;
  throw DataEngineError(
      DataEngineErrorNumber::datetime_dont_support_desired_values_yet,
      QString("Dataengine: Datetime fields dont support desired values yet. "
              "Field_name:  %1")
          .arg(field_name));
}

void DateTimeDataEntry::set_desired_value_from_actual(
    DataEngineDataEntry *from) {
  (void)from;
  throw DataEngineError(
      DataEngineErrorNumber::datetime_dont_support_desired_values_yet,
...
bool DateTimeDataEntry::is_desired_value_set() const { return false; }
...
QString DateTimeDataEntry::get_specific_json_name() const { return "datetime"; }
...
BoolDataEntry::BoolDataEntry(const FormID name,
                             std::experimental::optional<bool> desired_value,
                             QString description)
    : DataEngineDataEntry(name), desired_value(std::move(desired_value)),
      description(description) {}
...
QString BoolDataEntry::get_description() const { return description; }
...
QString BoolDataEntry::get_unit() const { return ""; }

double BoolDataEntry::get_si_prefix() const { return 1; }

double BoolDataEntry::get_desired_number() const {
  throw DataEngineError(
      DataEngineErrorNumber::desired_value_is_not_a_number,
      QString("Dataengine: Desired value of field %1 not a number, it is bool")
          .arg(field_name));
}

bool BoolDataEntry::compare_unit_desired_siprefix(
    const DataEngineDataEntry *from) const {
...
void BoolDataEntry::set_actual_value(bool value) { this->actual_value = value; }
...
bool BoolDataEntry::is_desired_value_set() const { return (bool)desired_value; }
...
QString BoolDataEntry::get_specific_json_name() const { return "bool"; }

ReferenceDataEntry::ReferenceDataEntry(const ReferenceDataEntry &other)
    : DataEngineDataEntry{other.field_name}, tolerance{other.tolerance},
      description{other.description}, reference_links{other.reference_links},
  ...
  if (entry_num) {
    ...
    assert(0);
    // unknown data type
    // reference_type would not be allowed since ther is no recursion(reference
    // pointing to a reference)
  }
}

ReferenceDataEntry::ReferenceDataEntry(const FormID name,
                                       QString reference_string,
                                       NumericTolerance tolerance,
                                       QString description)
    : DataEngineDataEntry(name), tolerance(tolerance),
      description(description) {
  ...
  if (reference_links[0].value == ReferenceLink::ReferenceValue::DesiredValue) {
    ...
    if ((bool)target_instance_count == false) {
      ...
      throw DataEngineError(
          ...
          QString("Dataengine: References must not point to the actual value "
                  "of an undefined instance. The referencing fieldname: \"%1\"")
              .arg(field_name));
    }
    if (target_instance_count.value() != 1) {
      throw DataEngineError(
          ...
          QString(
              "Dataengine: References must not point to the actual value of "
              "multi-instance-fields. The referencing fieldname: \"%1\", the "
...
void ReferenceDataEntry::set_actual_value(double number) {
  ...
  if (!num_entry) {
    throw DataEngineError(
        ...
        QString("Dataengine: The referencing field \"%1\" is not a number as "
                "it must be if you set it with the number: \"%2\"")
            .arg(field_name)
            .arg(number));
  }
...
void ReferenceDataEntry::set_actual_value(QString val) {
  ...
  if (!text_entry) {
    throw DataEngineError(
        DataEngineErrorNumber::setting_reference_actual_value_with_wrong_type,
        QString("Dataengine: The referencing field \"%1\" is not a string as "
                "it must be if you set it with the string: \"%2\"")
            .arg(field_name)
            .arg(val));
  }
...
void ReferenceDataEntry::set_actual_value(DataEngineDateTime val) {
  ...
  if (!datetime_entry) {
    throw DataEngineError(
        DataEngineErrorNumber::setting_reference_actual_value_with_wrong_type,
        QString("Dataengine: The referencing field \"%1\" is not a datetime as "
                "it must be if you set it with the datetime: \"%2\"")
            .arg(field_name)
            .arg(val.str()));
  }
...
void ReferenceDataEntry::set_actual_value(bool val) {
  ...
  if (!bool_entry) {
    throw DataEngineError(
        DataEngineErrorNumber::setting_reference_actual_value_with_wrong_type,
        QString("Dataengine: The referencing field \"%1\" is not a bool as it "
                "must be if you set it with the bool: \"%2\"")
            .arg(field_name)
            .arg(val));
  }
...
QString ReferenceDataEntry::get_actual_values() const {
  assert_that_instance_count_is_defined();
  if (entry ==
      nullptr) { // this happens if for any reason the construction of the
                 // dataengine fails. (like forgotten tolerance) Then the
                 // exceptional approval tries to collect data over dead fields
    return unavailable_value;
  }
...
bool ReferenceDataEntry::compare_unit_desired_siprefix(
    const DataEngineDataEntry *from) const {
  (void)from;
  assert(0);
  return false;
...
void ReferenceDataEntry::dereference(DataEngineSections *sections,
  ...
  if (reference_links.size() == 0) {
    throw DataEngineError(
        DataEngineErrorNumber::reference_not_found,
        QString("Dataengine: reference non existing. Fieldname: \"%1\"")
            .arg(field_name));

  } else if (reference_links.size() > 1) {
    if (!is_dummy_mode) { // dummy mode means that no dependency tags were
                          // defined in order to get just the structure of the
                          // data engine source file. for example for report
                          // templates or report queries.
                          // hence we can ignore this ambiguous case.
      QString t;
      ...
      throw DataEngineError(DataEngineErrorNumber::reference_ambiguous,
                            QString("Dataengine: reference ambiguous  with "
                                    "links: \"%1\", fieldname: \"%2\"")
                                .arg(t)
                                .arg(field_name));
    }
  ...
  auto targets =
      sections->get_entries_const(reference_links[0].link, is_dummy_mode);
  ...
  if (have_entries_equal_desired_values(targets)) {
    ...
    if (is_dummy_mode) {
      ...
      throw DataEngineError(
          ...
          QString("Dataengine: Reference \"%1\" points to multiinstance with "
                  "different values. This is not allowed.")
              .arg(field_name));
    }
  }
  ...
  if (reference_links[0].value == ReferenceLink::ReferenceValue::DesiredValue) {
    if (!entry_target->is_desired_value_set()) {
      throw DataEngineError(
          DataEngineErrorNumber::reference_target_has_no_desired_value,
          QString("Dataengine: reference \"%1\" points to desired value "
                  "\"%2\", even though no desired value is defined in \"%2\".")
              .arg(field_name)
  ...
  if (num_entry) {
    std::experimental::optional<double>
        temp_desired_value; // will be set later, when beeing compared
    if (!tolerance.is_defined()) {
      throw DataEngineError(
          DataEngineErrorNumber::reference_is_a_number_and_needs_tolerance,
          QString("Dataengine: reference \"%1\" pointing to \"%2\", is a "
                  "number but has no tolerance defined. A tolerance must be "
                  "defined on numbers.")
              .arg(field_name)
              .arg(reference_links[0].link));
    }
    ...
    entry = std::make_unique<NumericDataEntry>(
        "", temp_desired_value, tolerance, num_entry->unit,
        num_entry->get_si_prefix(), description);
  } else if (text_entry) {
    std::experimental::optional<QString>
        temp_desired_value; // will be set later, when beeing compared
    entry =
        std::make_unique<TextDataEntry>("", temp_desired_value, description);
  } else if (bool_entry) {
    std::experimental::optional<bool>
        temp_desired_value; // will be set later, when beeing compared
    entry =
        std::make_unique<BoolDataEntry>("", temp_desired_value, description);
    ...
    throw DataEngineError(
        ...
        QString("Dataengine: Datetime type does not support references yet "
                "since it neither supports values. Fieldname: \"%1\"")
            .arg(field_name));
  ...
  if (!num_entry) {
    if (tolerance.is_defined()) {
      throw DataEngineError(
          ...
          QString(
              "Dataengine: reference \"%1\" pointing to \"%2\", is not a "
              "number but has a tolerance defined. A tolerance is only allowed "
  ...
  for (auto &ref : refs) {
    ...
    if (ref.endsWith(ACTUAL_VALUE)) {
      ...
      throw DataEngineError(
          ...
          QString("Dataengine: reference \"%1\" target declaration must end "
                  "with \".desired\" or \".actual\" but does not.")
              .arg(field_name));
    }
...
void ReferenceDataEntry::assert_that_instance_count_is_defined() const {
  if (not_defined_yet_due_to_undefined_instance_count) {
    throw DataEngineError(
        ...
        QString("Dataengine: reference \"%1\" can not be used since it is "
                "pointing to an instance which is undefinded")
            .arg(field_name));
  }
}

void ReferenceDataEntry::set_desired_value_from_desired(
    DataEngineDataEntry *from) {
  (void)from;
  assert(0);
}

void ReferenceDataEntry::set_desired_value_from_actual(
    DataEngineDataEntry *from) {
  (void)from;
  assert(0);
}
...
void DataEngineActualValueStatisticFile::start_recording(QString file_root_path,
                                                         QString file_prefix) {
...
QString select_newest_file_name(QStringList file_list, QString prefix) {
  ...
  for (const QString &file_name : file_list) {
    QDateTime dt = decode_date_time_from_file_name(file_name.toStdString(),
                                                   prefix.toStdString());
    //  qDebug() << dt.toString();
    if (dt > dt_max) {
...
QString DataEngineActualValueStatisticFile::select_file_name_to_be_used(
    QStringList file_list) {
  return select_newest_file_name(file_list, file_prefix);
}

void DataEngineActualValueStatisticFile::save_to_file() { close_file(); }
...
void DataEngineActualValueStatisticFile::open_or_create_new_file() {
  ...
  QString file_name =
      select_file_name_to_be_used(directory.entryList(nameFilter));
...
void DataEngineActualValueStatisticFile::create_new_file() {
  QString file_name =
      QString::fromStdString(propose_unique_filename_by_datetime(
          file_root_path.toStdString(), file_prefix.toStdString(), ".json"));
  open_file(file_name);
}
...
void DataEngineActualValueStatisticFile::close_file() {
  if (is_opened && used_file_name.size()) {
    ...
    if (!saveFile.open(QIODevice::WriteOnly)) {
      throw DataEngineError(DataEngineErrorNumber::cannot_open_file,
                            QString{"Dataengine: Can not open file: \"%1\" for "
                                    "saving actual value statistics."}
                                .arg(used_file_name));
      remove_lock_file(); // TODO: This will never be executed. Do we need to
                          // remove the lock file or not?
      return;
    }
...
void DataEngineActualValueStatisticFile::open_file(QString file_name) {
  ...
  if (check_and_create_lock_file()) {
    used_file_name = QString::fromStdString(propose_unique_filename_by_datetime(
        file_root_path.toStdString(), file_prefix.toStdString(), ".json"));
  }
...
void DataEngineActualValueStatisticFile::set_actual_value(
    const FormID &field_name, const QString serialised_dependency,
    double value) {
  if (is_opened) {
    QJsonObject obj{};
...
void DataEngineActualValueStatisticFile::set_dut_identifier(
    QString dut_identifier) {
  this->dut_identifier = dut_identifier;
}

DataEngineDateTime::DataEngineDateTime(QString text) {
  // another common date string: vc.h/GITDATE: "2019-01-15" (covered by
  // datetime_from_string) another common date string:
  // report_dump/general/datetime_str "yyyy:MM:dd HH:mm:ss"; another common date
  // string: git_info().date: 2018-12-19 12:50:01 +0100 another common date
  // string: scpidevice:get_calibration(): "yyyy.MM.dd"
...
QDateTime DataEngineDateTime::dt() const { return dt_m; }
...
QList<DateTimeFormatPrecision> DataEngineDateTime::formats() {
  QList<DateTimeFormatPrecision> result{
      ...
      // another common date string: report_dump/general/datetime_str
      // "yyyy:MM:dd HH:mm:ss";
      ...
      {{QString("yyyy.MM.dd"), DateTimeFormatPrecision::date}, //
       ...
       {QString("yyyy-MM-dd hh:mm:ss.zzz"),
        DateTimeFormatPrecision::date_time_ms}} //
  };
  return result;
}

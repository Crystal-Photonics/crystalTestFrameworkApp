...
QString DataEngineSection::get_sql_instance_name() const {
	return get_section_name() + "_instance";
}
...
void Data_engine::generate_tables() const {
	...
	for (auto &section : sections.sections) {
		//Data Band Header
		{
			...
			XML{"geometry"}.attributes({{"width", "2000"}, {"height", "150"}, {"x", "0"}, {"y", QString::number(XML::y() += 100)}, {"Type", "QRect"}});
			XML{"objectName"}.attribute("Type", "QString").value(QString{"DataBandHeader_%1"}.arg(section.get_section_name()));
		...
		//Data Band
		{
			...
			XML{"datasource"}.attribute("Type", "QString").value(section.get_sql_instance_name());
			{
				...
				children.attribute("Type", "Collection");
				{
					...
					XML{"objectName"}.attribute("Type", "QString").value("DataSectionName_" + section.get_section_name());
				}
			}
		}
		//Data Band Subdetail
	...
	while (xml_in.readNext() != QXmlStreamReader::Invalid) {
		if (xml_in.isStartElement()) {
			if (xml_in.name() == "datasourcesManager") {
				...
				add_sources_to_form(database_path);
				xml_out.writeCurrentToken(xml_in);
				continue;
			}
		}
...
void Data_engine::add_sources_to_form(const std::string &data_base_path) const {
	...
	const auto connection_name = "default_connection";
	{
		...
		XML{"databaseName"}.attribute("Type", "QString").value(data_base_path.c_str());
	}
	//queries
	{
		XML queries{"queries"};

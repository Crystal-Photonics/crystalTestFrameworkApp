...
void Data_engine::set_source(std::istream &source) {
    ...
    for (const auto &section : sections.sections) { // lets test whether field names with the same name
...
DecodecFieldID DataEngineSections::decode_field_id(const FormID &id) {
    ...
    auto names = id.split("/");
    if (names.count() != 2) {
        throw DataEngineError(DataEngineErrorNumber::faulty_field_id, QString("Dataengine: field id needs to be in format "
...
const VariantData *DataEngineInstance::get_variant() const {
    ...
    throw DataEngineError(DataEngineErrorNumber::no_variant_found, QString("Dataengine: No dependency fullfilling variant "
                                                                           "found in section: \"%1\"")
                                                                       .arg(section_name));
}
...
void DataEngineInstance::delete_unmatched_variants(const QMap<QString, QList<QVariant>> &tags, uint instance_index, uint instance_count) {
    ...
    if (variants.size() > 1) {
        throw DataEngineError(DataEngineErrorNumber::non_unique_desired_field_found, QString("Dataengine: More than one dependency fullfilling variants "
    ...
    if (object.isArray()) {
        ...
        if (obj.contains("instance_count")) {
            ...
            if ((!is_string) && (!is_number)) {
                throw DataEngineError(DataEngineErrorNumber::wrong_type_for_instance_count, QString("Dataengine: Instance count of section \"%1\" must not be "
        ...
        if (obj.contains("allow_empty_section")) {
            if (obj["allow_empty_section"].isBool()) {
                ...
                throw DataEngineError(DataEngineErrorNumber::allow_empty_section_with_wrong_type, QString("Dataengine: \"Allow_empty_section\" -tag of section "
...
bool DataEngineSection::set_instance_count_if_name_matches(QString instance_count_name, uint instance_count) {
    ...
    if (this->instance_count_name == instance_count_name) {
        if (is_section_instance_defined()) {
            throw DataEngineError(DataEngineErrorNumber::instance_count_already_defined, QString("Dataengine: Instance count of section \"%1\" is already "
...
void DataEngineSection::assert_instance_is_defined() const {
    if (is_section_instance_defined() == false) {
        throw DataEngineError(DataEngineErrorNumber::instance_count_yet_undefined, QString("Dataengine: Instance count of section \"%1\" yet undefined. "
...
void DataEngineSection::use_instance(QString instance_caption, uint instance_index) {
    ...
    if (instance_index >= instance_count.value()) {
        throw DataEngineError(DataEngineErrorNumber::instance_count_exceeding, QString("Dataengine: Instance index (%1) of section "
    ...
    if (keys_to_test.count()) {
        ...
        if ((instance_count != length_of_first_tag_value_list.value()) && (length_of_first_tag_value_list.value() != 1)) {
            throw DataEngineError(DataEngineErrorNumber::instance_count_must_match_list_of_dependency_values,
                                  QString("Dataengine: Instance count of section \"%1\" must have the same "
...
void DataEngineSection::set_actual_datetime(const FormID &id, DataEngineDateTime value) {
    ...
    if (!value.isValid()) {
        throw DataEngineError(DataEngineErrorNumber::datetime_is_not_valid, QString("Dataengine: Date time value for the field \"%1\" is not "
...
void Data_engine::save_to_json(QString filename) {
    ...
    if (!saveFile.open(QIODevice::WriteOnly)) {
        throw DataEngineError{DataEngineErrorNumber::cannot_open_file, QString{"Dataengine: Can not open file: \"%1\" for "
    ...
    if (reference_links[0].value == ReferenceLink::ReferenceValue::DesiredValue) {
        ...
        if (target_instance_count.value() != 1) {
            throw DataEngineError(DataEngineErrorNumber::reference_must_not_point_to_multiinstance_actual_value,
                                  QString("Dataengine: References must not point to the actual value of "
...
QString ReferenceDataEntry::get_actual_values() const {
    assert_that_instance_count_is_defined();
    if (entry == nullptr) { // this happens if for any reason the construction of the
...
void ReferenceDataEntry::dereference(DataEngineSections *sections, const bool is_dummy_mode) {
    ...
    if (reference_links.size() == 0) {
        ...
        if (!is_dummy_mode) { // dummy mode means that no dependency tags were
            ...
            throw DataEngineError(DataEngineErrorNumber::reference_ambiguous, QString("Dataengine: reference ambiguous  with "
                                                                                      "links: \"%1\", fieldname: \"%2\"")
                                                                                  .arg(t)
                                                                                  .arg(field_name));
        }
    ...
    if (num_entry) {
        ...
        if (!tolerance.is_defined()) {
            throw DataEngineError(DataEngineErrorNumber::reference_is_a_number_and_needs_tolerance,
        ...
        throw DataEngineError(DataEngineErrorNumber::datetime_dont_support_references_yet, QString("Dataengine: Datetime type does not support references yet "
    ...
    if (!num_entry) {
        if (tolerance.is_defined()) {
            throw DataEngineError(DataEngineErrorNumber::reference_is_not_number_but_has_tolerance,
                                  QString("Dataengine: reference \"%1\" pointing to \"%2\", is not a "
...
void ReferenceDataEntry::parse_refence_string(QString reference_string) {
    ...
    for (auto &ref : refs) {
        ...
        if (ref.endsWith(ACTUAL_VALUE)) {
            ...
            throw DataEngineError(DataEngineErrorNumber::illegal_reference_declaration, QString("Dataengine: reference \"%1\" target declaration must end "

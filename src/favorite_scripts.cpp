...
void FavoriteScripts::load_from_file(QString file_name) {
  ...
  if (!QFile::exists(file_name)) {
    Utility::thread_call(MainWindow::mw, [file_name] {
      QMessageBox::warning(
          MainWindow::mw, QObject::tr("Can't open favorite scripte file"),
          QObject::tr(
              "Can't open favorite scripte file. File does not exist: ") +
              file_name);
    });
  ...
  if (j_doc.isNull()) {
    Utility::thread_call(MainWindow::mw, [file_name] {
      QMessageBox::warning(
          MainWindow::mw,
          QObject::tr("could not parse file with favorite scripts"),
          QObject::tr("could not parse file with favorite scripts. Seems the "
                      "json is broken: ") +
              file_name);
    });
    return;
...
bool FavoriteScripts::save_to_file(QList<ScriptEntry> script_entries) {
  ...
  if (!saveFile.open(QIODevice::WriteOnly)) {
    Utility::thread_call(MainWindow::mw, [this] {
      QMessageBox::warning(
          MainWindow::mw,
          QObject::tr("Could not save file with favorite scripts"),
          QObject::tr("Could not save file for favorite scripts. Is it write "
                      "protected?\n") +
              file_name_m);
    });
    return false;
...
bool FavoriteScripts::set_alternative_name(QString script_path,
